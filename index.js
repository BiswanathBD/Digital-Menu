  ...
  try {
    ...
    categoryData.forEach((category) => {
      ...
      <div id="${category.id}"  onclick="showCategoryData(${category.id})" class="group flex justify-between items-center gap-2 bg-gradient-to-l from-yellow-600 p-1 rounded-r-full mr-2">
              <p class="font-semibold text-lg text-white ml-6 group-hover:ml-7 group-hover:text-xl transition-all">${category.categoryName}</p>
...
const showCategoryData = async (id) => {
  try {
    ...
    foodsData.forEach((food) => {
      foodContainer.innerHTML += `
      <div class="bg-yellow-600/40 border backdrop-blur-sm p-2 rounded-xl mb-4 mr-2">
